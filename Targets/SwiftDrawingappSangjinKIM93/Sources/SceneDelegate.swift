...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func createDrawingViewModel() -> DrawingViewModelProtocol {
        ...
        let drawingUsecase = DrawingUseCase(drawingStore: drawingStore, drawingFactory: drawingFactory)
        
        let socketManager = SocketManager(host: "localhost", port: 9090)
        let chatServerClient = ChatServerClient(socket: socketManager)
        
        let chatUseCase = ChatUseCase(drawingStore: drawingStore, chatServerClient: chatServerClient)
